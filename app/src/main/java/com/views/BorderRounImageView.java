...
/**
 * 自定义 圆形/圆角 图片控件   使用方式与ImageView 基本一致
 * className:圆角带有边框imageView
 * author:
 * description:
 * date: 2017/8/11 15:23
 * update: 2017/8/11
 * version:
 */
public class BorderRounImageView extends ImageView {
    ...
    private int mBorderWidth = 2;  //默认2
    private int mRectRoundRadius;
    private boolean square = false; //是否设置宽度和高度相等
    ...
    public BorderRounImageView(Context context) {
        super(context);
    }

    public BorderRounImageView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initParmas(context, attrs);
    }

    public BorderRounImageView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initParmas(Context context, AttributeSet attrs) {
        TypedArray arr = context.obtainStyledAttributes(attrs, R.styleable.BorderRoundImageView);//获取  自定义 控件的属性
        if (arr != null) {
            mType = arr.getInt(R.styleable.BorderRoundImageView_type, DEFAULT_TYPE);
            mBorderColor = arr.getColor(R.styleable.BorderRoundImageView_borderColor, DEFAULT_BORDER_COLOR);
            mBorderWidth = arr.getDimensionPixelSize(R.styleable.BorderRoundImageView_myborderWidth, dip2px(DEFAULT_BORDER_WIDTH));
            mRectRoundRadius = arr.getDimensionPixelSize(R.styleable.BorderRoundImageView_rectRoundRadius, dip2px(DEFAULT_RECT_ROUND_RADIUS));
            square = arr.getBoolean(R.styleable.BorderRoundImageView_square, false);
            arr.recycle();
        }
    }
